...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <belcoinprivkey> [label] [rescan=true]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <belcoinaddress>\n"
            "Reveals the private key corresponding to <belcoinaddress>.");
...
Value dumppem(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "dumppem <belcoinaddress> <filename> <passphrase>\n"
            "Dump the key pair corresponding to <belcoinaddress> and store it as encrypted PEM file."
            + HelpRequiringPassphrase());
