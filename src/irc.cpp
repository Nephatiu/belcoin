...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #belcoinTEST2\r");
            Send(hSocket, "WHO #belcoinTEST2\r");
        } else {
            // randomly join #belcoin00-#belcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #belcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #belcoin%02d\r", channel_number).c_str());
        }
