...
static const char *strDNSSeed[][2] = {
    {"node.belcoin.karelia.pro", "dnsseed.belcoin.karelia.pro"},
    {"belcoin.ru", "dnsseed.belcoin.ru"},
    {"belcoin.ru", "testseed.belcoin.ru"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BelCoin is probably already running."), addrBind.ToString().c_str());
        else
